...
<nav id="navLinks" class="hidden md:flex md:items-center md:space-x-3">
    ...
    <div class="relative">
        ...
        @if (Auth::user()->cart)
            <span class="absolute inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-black transform translate-x-1/2 -translate-y-1/2 bg-yellow-500 rounded-full right-1 top-2">{{ Auth::user()->cart->count() }}</span>
        @else
        ...
        <div id="cartDropdown" class="absolute right-0 z-50 hidden h-auto mt-2 text-black bg-white rounded-lg shadow-lg w-96">
            <div class="p-4">
                <p class="text-lg font-bold">Total: {{ Auth::user()->cart ? Auth::user()->cart->sum('quantity') : 0 }}</p>
                @forelse(Auth::user()->cart as $item)
                    <div class="max-w-xs p-2 mb-2 border border-red-500 rounded-lg">
                        <div class="relative">
                            <!-- Product Image -->
                            <img src="{{ asset('storage/ProductImages/' . $item->product->upload_part_image) }}" alt="image" class="object-cover w-16 h-16 rounded-md">
                            <!-- Heart Icon -->
